...
interface ByPathValue<Data extends StoreData> {
	<TPath extends Path.From<Data>, Value extends Path.At<Data, TPath>>(path: TPath, value: Value): Promise<void>;
}
...
>(data: TData, path: TPath, value: TValue, separator?: TSep): TData {
	...
	const $path = separator ? path.toString().replace(RegExp(separator, 'g'), '.') : path.toString();
